 ...
 */
public class GeneratePatternThread extends NotificationThread {
	...
	public GeneratePatternThread(ArrayList<JGraphtClosureGraph> cGraphList, ArrayList<Integer> closureIndex,
			ArrayList<String> edgeLabel, ArrayList<Float> edgeLabelWt, int minSize, int maxSize, int threadNum,
			GeneratePatternThreadCompleteListener listener, ArrayList<Integer> GUIPatternSize,
		...
		for (int i = 0; i < candidateClosureIndex.size(); i++) {
			...
			for (int s = patternMinSize; s <= maxSize; s++) {
				...
				if (patternSizeCounter.get(patternIndex) < maxPatternForEachSize) {
					ArrayList<closureEdge> p = cGraph.getBestPattern(s, false, DBVertexLabel, DBVertexLabelValency);
					//if (p.size() > 0)
