...
public class KSTest {
    ...
    private static double invqks(double q) {
        ...
        if (q > 0.3) {
            double f = -0.392699081698724155 * Math.sqrt(1. - q);
            double y = invxlogx(f);
            double t;
            do {
                double logy = Math.log(y);
                double ff = f / Math.sqrt(1. + Math.pow(y, 4) + Math.pow(y, 12));
    ...
    public KSTest TestTwoArrayList(ArrayList<Double> x,ArrayList<Double> y) {
    	Collections.sort(x);
    	Collections.sort(y);
        ...
        double d1, d2, dt, fn1 = 0.0, fn2 = 0.0;
        while (j1 < n1 && j2 < n2) {
            if ((d1 = x.get(j1)) <= (d2 = y.get(j2))) {
                do {
                    fn1 = ++j1 / en1;
                } while (j1 < n1 && d1 == x.get(j1));
            }
            if (d2 <= d1) {
                do {
                    fn2 = ++j2 / en2;
                } while (j2 < n2 && d2 == y.get(j2));
            }
            if ((dt = Math.abs(fn2 - fn1)) > d) {
                d = dt;
            }
    ...
    public  static void main(String[] args) {
    	KSTest test = new KSTest(0,0);
    	double[] x1 =  {4,4,5,3, 3,3,4,5,5};
    	ArrayList<Double> ax1 = new ArrayList<Double> ();
    	for(int i=0; i<9; i++)  ax1.add(x1[i]);
    	double[] x2 =  {3, 3,3,4,5,4,5,5,5};
    	ArrayList<Double> ax2 = new ArrayList<Double> ();
    	for(int i=0; i<9; i++)  ax2.add(x2[i]);
    	double[] x3=   {3, 3,3,4,4,5,5,5,5};
    	ArrayList<Double> ax3= new ArrayList<Double> ();
    	for(int i=0; i<9; i++)  ax3.add(x3[i]);
    	double[] x4=   {3, 3,4,4,4,5,5,5,5};
    	ArrayList<Double> ax4 = new ArrayList<Double> ();
    	for(int i=0; i<9; i++)  ax4.add(x4[i]);
    	double[] x5=   {4, 4,4,4,4,5,5,5,5};
    	ArrayList<Double> ax5= new ArrayList<Double> ();
    	for(int i=0; i<9; i++)  ax5.add(x5[i]);
    	KSTest test1 =  test.test(x1, x2);
    	System.out.println(test1.d);
    	KSTest test2 = test.test(x1, x3);
    	System.out.println(test2.d);
    	KSTest test3 = test.test(x1, x4);
    	System.out.println(test3.d);
    	KSTest test4 = test.test(x1, x5);
    	System.out.println(test4.d);
    	
    	test1 =  test.TestTwoArrayList(ax1, ax2);
    	System.out.println(test1.d);
    	test2 =  test.TestTwoArrayList(ax1, ax3);
    	System.out.println(test2.d);
    	test3 =  test.TestTwoArrayList(ax1, ax4);
    	System.out.println(test3.d);
    	test4 =  test.TestTwoArrayList(ax1, ax5);
    	System.out.println(test4.d);
