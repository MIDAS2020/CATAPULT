...
void FrequentTreeList::populate2(const vector<ptrFreeTree>& database)
{
	...
	for (long s = 1; s < database.size(); s++)
	{
		...
		{
			//cout << pft->tid << " " << pft->vCount << endl;
			t = pft->adj[i];
			//cout <<"index:"<< i << "->" << t->v << "   label:"<< pft->vertexLabel[i] << "->" << pft->vertexLabel[t->v] << endl;
			while (t != 0)
			{
...
map<CanonicalTree, supportNode> FrequentTreeList::getfrequentList() {

	return frequentList;
}

void FrequentTreeList::printfrequentList(ostream& out)
{
	map<CanonicalTree, supportNode>::iterator it;
	for (it = frequentList.begin(); it != frequentList.end(); it++) {
		//out << "*************111" << endl;
		CanonicalTree ctree = it->first;
		out << ctree;
		//out << "*************222" << endl;
		//supportNode   snode = it->second;
		//out << snode.lastTid << " " << snode.support << endl;
		//out << "......................" << endl;

		//for (list<long>::iterator i = snode.tidList.begin(); i != snode.tidList.end(); i++) {
		////	out << *i << " ";
		//}
