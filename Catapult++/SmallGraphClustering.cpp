...
SmallGraphClustering::SmallGraphClustering(ofstream& logfile)
{
	...
	logfile << "Small graph clustering time (Original): " << (smallclusterend - smallclusterstart) << endl;
	...
	if (is_fctAsFeatureVactor) {
		...
		smallclusterend2 = clock();
		logfile << "Small graph clustering time (Maintenance): " << (smallclusterend2 - smallclusterstart2) << endl;
	}
