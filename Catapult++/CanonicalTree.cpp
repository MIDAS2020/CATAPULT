	...
	: FreeTree(rhs)
{
	...
	/*
	for (int i = 0; i < canonicalString.size();i++)
...
void CanonicalTree::findRoot()
{
	...
	//cout << "sss root:" << root1 << "---" << root2 << endl;
}
...
{
	out << "t # " << COUNTTID << " "<< rhs.vCount << endl;
	for (short i = 1; i <= rhs.vCount; i++)
	{
		out << "v " << i - 1 << " " << rhs.vertexLabel[i] - 1 << endl;
	}
...
ostream& operator<<(ostream& out, const CanonicalTree& rhs)
{
	...
	for (short i = 1; i <= rhs.vCount; i++)
	{
		out << rhs.vertexLabel[i] << ' ' << rhs.degree[i] << ':';
		TnodeLink n = rhs.adj[i];
		while (n != 0)
		{
			out << "--" << n->v << '_' << n->eLabel << ' ';
