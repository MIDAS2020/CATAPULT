 ...
 *      ClosedTrees.cpp
 *
 *      Copyright (C) 2008 Universitat Polit¨¨cnica de Catalunya
 *      @author Albert Bifet - Ricard Gavald¨¤ (abifet,gavalda@lsi.upc.edu)
 *
...
void ClosedTrees::insert(TreeNodes* tn, int iSupport, bool blUpdate) {
	...
	if (mis_Adaptive2) {
		...
		cout << "RelativeSupport:" << msupport_factory->RelativeSupport(iSupport) << "," << iSupport << endl;
	...
	for (mapTreeIterator p = LatticeTreeList->lower_bound(l); p != LatticeTreeList->upper_bound(l); p++) {
		if (p->second->get_TreeNodes_() == tn) {
			...
			if (p->second->get_Support() != NewSupport) {
				...
				cout << "XXXXXXXXXXXXXXXXXXXXXXXXXXXNewSupport:" << NewSupport << endl;
